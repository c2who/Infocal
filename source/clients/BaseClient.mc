...
class BaseClientHelper {
  ...
  public static function calcNeedsDataUpdate(
    type as String,
    update_interval_secs as Number
  ) as Boolean {
    ...
    if (lastTime == null) {
      ...
      var remain =
        ((Time.now().value() - lastTime) / Gregorian.SECONDS_PER_MINUTE) %
        _retry_backoff_minutes;
      return remain == 0;
    } else if (
      data == null ||
      data["clientTs"] < Time.now().value() - update_interval_secs
    ) {
  ...
  public static function storeData(data as Dictionary<String, Number>) as Void {
    ...
    if (responseCdoe == 200) {
      ...
      props.setValue(type + Globals.DATA_TYPE_ERROR_SUFFIX, null);
      props.setValue(type + Globals.DATA_TYPE_RETRIES_SUFFIX, null);
    } else {
      // return error to the caller, and update retries count
      var retries = props.getValue(type + Globals.DATA_TYPE_RETRIES_SUFFIX);
      retries = retries == null ? 1 : retries + 1;

      props.setValue(type + Globals.DATA_TYPE_ERROR_SUFFIX, data);
      props.setValue(type + Globals.DATA_TYPE_RETRIES_SUFFIX, retries);
    }
  }
  ...
  public static function getCommunicationsErrorCodeText(
    responseCode as Number
  ) as String {
