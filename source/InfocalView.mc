...
class InfocalView extends WatchUi.WatchFace {
  ...
  function onUpdate(screenDc) {
   ...
   public function onPartialUpdate(dc as Dc) as Void {
    ...
    if (
      always_on_digi_font != null &&
      Application.getApp().getProperty("always_on_second")
    ) {
      var clockTime = System.getClockTime();
    ...
    if (
      always_on_digi_font != null &&
      Application.getApp().getProperty("always_on_heart")
    ) {
  ...
  function updateColorsInUse() {
    ...
    if (gtheme != theme_code || theme_code == 18) {
      if (theme_code == 18) {
        ...
        var text_color = Application.getApp().getProperty("text_color");
        var accent_color = Application.getApp().getProperty("accent_color");
        var ticks_color = Application.getApp().getProperty("ticks_color");
        var bar_background_color = Application.getApp().getProperty(
          "bar_background_color"
        );
        var bar_indicator_color = Application.getApp().getProperty(
          "bar_indicator_color"
        );
        var bar_graph_color_top = Application.getApp().getProperty(
          "bar_graph_color_top"
        );
        var bar_graph_color_bottom = Application.getApp().getProperty(
  ...
  function loadAlwaysOnFonts(dc as Graphics.Dc) {
    var always_on_second =
      Application.getApp().getProperty("always_on_second") as Lang.Boolean;
    var always_on_heart =
      Application.getApp().getProperty("always_on_heart") as Lang.Boolean;
    var always_on_style =
      Application.getApp().getProperty("always_on_style") as Lang.Number;
