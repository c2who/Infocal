...
class ArcTextComplication extends Ui.Drawable {
   ...
   function need_draw() {
      ...
      if (Properties.getValue("left_digital_info")) {
         ...
         var can_draw = !(angle == 2 && !Properties.getValue("use_analog"));
         return dt_field.need_draw() && can_draw;
      }
   }
   ...
   function draw(dc) {
      ...
      if (need_draw()) {
         ...
         dc.setPenWidth(20);
         var target_r = barRadius - (((baseDegree < 180 ? 6 : -3) * center_x) / 120).toNumber();
         dc.drawArc(center_x, center_y, target_r, Graphics.ARC_CLOCKWISE, 360.0 - (baseDegree - 30.0), 360.0 - (baseDegree + 30.0));
   ...
   private function drawArcText(dc, text) {
      ...
      var KERNING_RATIOS =
         [
            ...
            0.68, // 'O'
            0.66, // 'P'
            0.8, // 'Q'  // TODO : Add letter Q to all arc text fonts
            0.67, // 'R'
            0.67, // 'S'
            0.55, // 'T'
            0.68, // 'U'
