...
class SunField extends BaseDataField {
   ...
   function getSunTimes(lat, lon, tz, tomorrow) as Array<Float?> {
      ...
      var jDate = d.day + Math.floor((153 * m + 2) / 5) + 365 * y + Math.floor(y / 4) - Math.floor(y / 100) + Math.floor(y / 400) - 32045;
      ...
      var C = 1.9148d * Math.sin(M * rad) + 0.02d * Math.sin(2 * M * rad) + 0.0003d * Math.sin(3 * M * rad);
      ...
      // Solar transit.
      var jTransit = 2451545.5d + jStar + 0.0053d * Math.sin(M * rad) - 0.0069d * Math.sin(2 * lambda * rad);

      // Declination of the sun.
      var delta = Math.asin(Math.sin(lambda * rad) * Math.sin(23.44d * rad));

      // Hour angle.
      var cosOmega = (Math.sin(-0.83d * rad) - Math.sin(lat * rad) * Math.sin(delta)) / (Math.cos(lat * rad) * Math.cos(delta));
      ...
      return [/* localRise */ deltaJRise * 24 + tzOffset, /* localSet */ deltaJSet * 24 + tzOffset];
   }
